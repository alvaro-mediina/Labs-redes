...
def get_response(connection, filename):
    ...
    if not check_http_response(header):
        ...
        if (header_str.startswith("HTTP/1.1 " + HTTP_REDIRECTS[0])
                or header_str.startswith("HTTP/1.1 " + HTTP_REDIRECTS[1])):
            ...
            while True:
                ...
                if line.lower().startswith("location:"):
                    ...
                    break
                if not line:  # Fin de los headers
                    break
...
def download(url, filename):
    """
    Descarga datos desde `url` usando HTTP y los guarda en `filename`.

    Si la respuesta HTTP indica una redirección (código 301 o 302), la función 
    sigue la redirección y descarga desde la nueva URL. Si la redirección es a HTTPS,
    la descarga se interrumpe.

    Excepciones:
        - Termina el programa si la dirección no es encontrada o la conexión falla.
...
def convert_ascii_char(unicode_url: str):
...
def main():
    ...
    if non_ascii_char(url):
        url = convert_ascii_char(url)
